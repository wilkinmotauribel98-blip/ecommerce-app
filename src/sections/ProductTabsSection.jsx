  ...
  return(
    <section className="my-5 flex flex-col gap-5  m-auto">
      <div className="text-white flex ">
        <button 
          className={`flex-1 py-3 cursor-pointer  ${tab == 'DESCRIPTION' && 'border-b-4 border-emerald-400'}`} 
          onClick={()=> setTab('DESCRIPTION')}
          aria-label="Description tab"
        >Description</button>
        <button 
          className={`flex-1 py-3 cursor-pointer ${tab == 'SPECIFICATIONS' && 'border-b-4 border-emerald-400'}`} 
          onClick={()=> setTab('SPECIFICATIONS')}
          aria-label="Specifitions tab"
          >Specifications</button>
        <button 
          className={`flex-1 cursor-pointer py-3 ${tab == 'REVIEWS' &&  'border-b-4 border-emerald-400'}`} 
